...
def train(rank, data_path, download, shuffle, batch_size, max_epochs, log_path, world_size, master_port, backend, timeout, num_classes, optimizer_lr, optimizer_weight_decay, patience):
    ...
    model = RegularizedNet(num_classes).to(rank)
...
if __name__ == '__main__':
    ...
    gpu_number = 0
    max_epochs = 20
    log_path = "/home/rostami/DMLS/main/Regularized_distributed_256_NCCL"
    optimizer_lr = 5e-4
    ...
    start_time = time.time()
    mp.spawn(train, nprocs=world_size, args=(data_path, download, shuffle, batch_size, max_epochs, log_path, world_size, master_port, backend, timeout, num_classes, optimizer_lr, optimizer_weight_decay, patience))
    end_time = time.time()
